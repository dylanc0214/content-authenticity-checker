...
export default function App() {
    ...
    const [inputText, setInputText] = useState(''); // Tracks initial user input OR paraphrased text
    ...
    // --- States for Paraphrasing ---
    const [isParaphrasing, setIsParaphrasing] = useState(false);
    const [paraphraseError, setParaphraseError] = useState(null);
    // REMOVED currentTextInEditor state and its useEffect
    ...
    // --- Updated Highlighting Function (Unchanged logic) ---
    ...
    // REMOVED useEffect for currentTextInEditor


    // --- Event Handlers ---
    const handleSubmit = async () => {
        // --- Get current text directly from the ref ---
        let currentText = '';
        if (inputRef.current) {
            currentText = inputRef.current.tagName === 'TEXTAREA' ? inputRef.current.value : inputRef.current.innerText;
        } else {
            currentText = inputText; // Fallback if ref not ready
        }
        // --- End change ---
        ...
        if (!currentText || !currentText.trim()) {
            ...
            // setCurrentTextInEditor(''); // No longer needed
            return;
        }
        ...
        // setCurrentTextInEditor(currentText); // No longer needed
    ...
    const handleParaphraseClick = async () => {
        ...
        if (inputRef.current) {
            // IMPORTANT: Use innerText to get text *without* HTML tags from the div
        ...
        try {
            const paraphrased = await callParaphraseApi(textToParaphrase); // Paraphrase the plain text
            ...
            setInputText(paraphrased); // Update the underlying state (used if user switches tabs etc.)
            // setCurrentTextInEditor(paraphrased); // No longer needed

            // Clear previous results
            setAiResult(null);
            setParaphraseResult(null);

            // Optional: Auto re-run check (commented out)
                ...
                <div className={`grid ${hasResult ? 'grid-cols-1 md:grid-cols-2 gap-x-8' : 'grid-cols-1'} transition-all duration-500 ease-in-out`}>
                    ...
                    <div className="p-6 md:p-8 flex flex-col">
                        ...
                        {hasResult || highlightedContent ? (
                            <div
                                ref={inputRef}
                                // REMOVED key={highlightedContent}
                            ...
                            <textarea
                                ...
                                onChange={(e) => {
                                    ...
                                    // setCurrentTextInEditor(e.target.value); // No longer needed
                                }}
                    ...
                    {hasResult && (
                        <div className="p-6 md:p-8 border-t md:border-t-0 md:border-l border-gray-700">
                            ...
                            {!isLoading && !isParaphrasing && activeTab === PARAPHRASE_CHECKER_TAB && paraphraseResult && ( <ParaphraseResultDisplay result={paraphraseResult} /> )}
                        </div>
                    )}
                </div>
            </div>
...
// --- Helper Components (Unchanged from previous versions) ---
...
function ParaphraseResultDisplay({ result }) { const score = result ? Math.round(result.plagiarismScore) : 0; const color = score > 15 ? 'text-red-400' : score > 5 ? 'text-yellow-400' : 'text-green-400'; const matchLevel = score > 15 ? 'High Match' : score > 5 ? 'Possible Match' : 'Likely Original'; return ( <div className="bg-gray-900/50 p-6 rounded-lg border border-gray-700 animate-fade-in h-full flex flex-col"> <div className="bg-yellow-900/50 border-l-4 border-yellow-500 text-yellow-300 p-4 rounded-md mb-4"> <h4 className="font-bold">Simulation Mode</h4> <p className="text-sm">This is simulated. Integrate a real plagiarism API for accurate checks.</p> </div> <h3 className="text-xl font-semibold text-white mb-4 text-center">Paraphrase Check Result</h3> <div className="flex items-center justify-center space-x-4 mb-4"> <div className={`text-6xl font-bold ${color}`}>{score}%</div> <div className="text-lg text-center"> <div className="font-semibold text-gray-200">Potential Plagiarism</div> <div className={`text-sm ${color}`}>{matchLevel}</div> </div> </div> <div className="w-full bg-gray-700 rounded-full h-4 overflow-hidden mb-4"> <div className={`h-4 rounded-full transition-all duration-500 ease-in-out ${ score > 15 ? 'bg-red-500' : score > 5 ? 'bg-yellow-500' : 'bg-green-500'}`} style={{ width: `${score}%` }}></div> </div> <div className="mt-6 flex-grow overflow-y-auto"> <h4 className="font-semibold text-gray-200 mb-2">Simulated Matched Sources:</h4> {result?.sources && result.sources.length > 0 ? ( <ul className="space-y-3">{result.sources.map((source, index) => (<li key={index} className="border border-gray-700 p-3 rounded-lg bg-gray-800"><a href={source.url} target="_blank" rel="noopener noreferrer" className="text-milk hover:underline font-medium block truncate text-sm">{source.url}</a><p className="text-gray-400 text-xs mt-1 italic">"{source.snippet}"</p><span className="text-xs font-semibold bg-red-800/50 text-red-300 px-2 py-0.5 rounded-full mt-2 inline-block">{source.matchPercent}% Match</span></li>))}</ul>) : (<p className="text-gray-500 text-sm italic">No simulated sources found.</p>)}</div></div>); }
